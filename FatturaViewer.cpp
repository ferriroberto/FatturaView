...
std::wstring FatturaViewer::GetWelcomePageHTML()
{
    ...
    html += L"body{font-family:Segoe UI,Arial;background:#2563eb;display:flex;align-items:center;justify-content:center;color:#fff}";
    html += L".c{text-align:center;padding:15px;background:rgba(255,255,255,0.15);border-radius:10px;max-width:450px}";
    html += L".i{width:40px;height:40px;margin:0 auto 5px}";
    html += L"h1{font-size:1.3em;margin-bottom:3px}";
    html += L".s{font-size:0.75em;opacity:.9;margin-bottom:10px}";
    html += L".st{background:rgba(255,255,255,0.2);padding:6px;margin:3px 0;border-radius:5px;text-align:left;display:flex;align-items:center;font-size:0.8em}";
    html += L".n{width:18px;height:18px;background:#fff;color:#2563eb;border-radius:50%;text-align:center;line-height:18px;font-weight:bold;margin-right:6px;flex-shrink:0;font-size:0.75em}";
    ...
    html += L"<rect x=\"25\" y=\"12\" width=\"55\" height=\"70\" rx=\"2\" fill=\"#fff\"/>";
    ...
    html += L"<polyline points=\"63,73 66,76 73,68\" stroke=\"#fff\" stroke-width=\"2\" fill=\"none\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/>";
    html += L"</svg>";
    html += L"<h1>FatturaView</h1>";
    html += L"<p class=\"s\">Visualizzatore Fatture Elettroniche PA</p>";
