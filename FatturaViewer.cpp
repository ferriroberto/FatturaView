...
std::wstring FatturaViewer::GetWelcomePageHTML()
{
    ...
    html += L"html,body{width:100%;height:100%;overflow:hidden;font-family:'Segoe UI',system-ui,-apple-system,sans-serif}";
    html += L"body{background:linear-gradient(135deg,#6366f1 0%,#a855f7 50%,#ec4899 100%);";
    html += L"display:flex;align-items:center;justify-content:center;position:relative;min-height:100vh}";
    ...
    html += L"@keyframes shimmer{0%{background-position:200% center}100%{background-position:-200% center}}";
    html += L".c{position:relative;z-index:1;text-align:center;padding:36px 32px;";
    html += L"background:#fff;border-radius:24px;max-width:520px;width:90%;";
    html += L"box-shadow:0 25px 50px -12px rgba(0,0,0,0.25),0 0 80px rgba(99,102,241,0.15);";
    ...
    html += L"h1{font-size:2.2em;margin-bottom:8px;font-weight:800;";
    html += L"background:linear-gradient(135deg,#6366f1 0%,#a855f7 100%);";
    html += L"-webkit-background-clip:text;-webkit-text-fill-color:transparent;";
    html += L"background-clip:text;letter-spacing:-0.03em}";
    html += L".s{font-size:0.95em;color:#64748b;font-weight:500;margin-bottom:4px}";
    html += L".bg{display:inline-flex;gap:6px;margin-top:8px;flex-wrap:wrap;justify-content:center}";
    html += L".b{display:inline-block;padding:4px 12px;border-radius:20px;font-size:0.75em;";
    html += L"font-weight:600;letter-spacing:0.02em}";
    html += L".b1{background:linear-gradient(135deg,#6366f1 0%,#8b5cf6 100%);color:#fff}";
    html += L".b2{background:linear-gradient(135deg,#10b981 0%,#059669 100%);color:#fff}";
    html += L".b3{background:linear-gradient(135deg,#f59e0b 0%,#d97706 100%);color:#fff}";
    html += L".st{background:linear-gradient(135deg,#f8fafc 0%,#f1f5f9 100%);";
    html += L"padding:14px 16px;margin:12px 0;border-radius:16px;text-align:left;display:flex;";
    html += L"align-items:center;font-size:0.92em;transition:all 0.35s cubic-bezier(0.4,0,0.2,1);";
    html += L"border:2px solid #e2e8f0;cursor:pointer;animation:slideUp 0.6s cubic-bezier(0.34,1.56,0.64,1) backwards}";
    ...
    html += L".st:hover{transform:translateX(6px) scale(1.02);";
    html += L"box-shadow:0 12px 24px -8px rgba(99,102,241,0.25);border-color:#a5b4fc;";
    html += L"background:linear-gradient(135deg,#eef2ff 0%,#e0e7ff 100%)}";
    html += L".n{width:36px;height:36px;background:linear-gradient(135deg,#6366f1 0%,#a855f7 100%);";
    html += L"color:#fff;border-radius:12px;display:flex;align-items:center;justify-content:center;";
    html += L"font-weight:800;margin-right:14px;flex-shrink:0;font-size:1em;";
    html += L"box-shadow:0 4px 12px rgba(99,102,241,0.4)}";
    html += L".st b{display:block;margin-bottom:3px;font-size:1.08em;color:#1e293b;font-weight:700}";
    html += L".st span.txt{line-height:1.5;color:#475569;font-size:0.95em}";
    ...
    html += L"<stop offset=\"0%\" style=\"stop-color:#6366f1\"/>";
    html += L"<stop offset=\"100%\" style=\"stop-color:#a855f7\"/>";
    html += L"</linearGradient>";
    html += L"<linearGradient id=\"g2\" x1=\"0%\" y1=\"0%\" x2=\"100%\" y2=\"100%\">";
    html += L"<stop offset=\"0%\" style=\"stop-color:#10b981\"/>";
    html += L"<stop offset=\"100%\" style=\"stop-color:#059669\"/>";
    html += L"</linearGradient>";
    ...
    html += L"<rect x=\"18\" y=\"18\" width=\"64\" height=\"74\" rx=\"4\" fill=\"#fff\" opacity=\"0.98\"/>";
    html += L"<path d=\"M 72 18 L 82 28 L 72 28 Z\" fill=\"#e0e7ff\"/>";
    html += L"<line x1=\"26\" y1=\"32\" x2=\"72\" y2=\"32\" stroke=\"#6366f1\" stroke-width=\"2.5\" opacity=\"0.8\"/>";
    html += L"<line x1=\"26\" y1=\"45\" x2=\"62\" y2=\"45\" stroke=\"#a855f7\" stroke-width=\"2.5\" opacity=\"0.6\"/>";
    html += L"<line x1=\"26\" y1=\"58\" x2=\"72\" y2=\"58\" stroke=\"#6366f1\" stroke-width=\"2.5\" opacity=\"0.5\"/>";
    html += L"<line x1=\"26\" y1=\"71\" x2=\"58\" y2=\"71\" stroke=\"#a855f7\" stroke-width=\"2.5\" opacity=\"0.4\"/>";
    html += L"<circle class=\"ic\" cx=\"75\" cy=\"80\" r=\"13\" fill=\"url(#g2)\" filter=\"url(#sh)\"/>";
    html += L"<polyline points=\"68,80 72,85 82,73\" stroke=\"#fff\" stroke-width=\"3.5\" fill=\"none\" stroke-linecap=\"round\" stroke-linejoin=\"round\"/>";
    html += L"</svg>";
    html += L"<div class=\"hd\">";
    ...
    html += L"</div></div>";
    html += L"<div class=\"st\"><div class=\"n\">1</div><div><b>Apri Archivio</b><span class=\"txt\">File - Apri archivio ZIP</span></div></div>";
    html += L"<div class=\"st\"><div class=\"n\">2</div><div><b>Seleziona Fattura</b><span class=\"txt\">Doppio click nella lista</span></div></div>";
    html += L"<div class=\"st\"><div class=\"n\">3</div><div><b>Applica Stile</b><span class=\"txt\">Ministero o Assosoftware</span></div></div>";
    html += L"<div class=\"st\"><div class=\"n\">4</div><div><b>Stampa PDF</b><span class=\"txt\">File - Stampa fatture</span></div></div>";
